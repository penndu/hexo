...
if (memo.APIVersion === 'new') {
    // 适配 Memos 0.30.0：原 ?parent=users/X 已废弃，改用 CEL filter
    const creatorFilter = `creator == "users/${memo.creatorId}"`;
    memoUrl = `${memos}/api/v1/memos?filter=${encodeURIComponent(creatorFilter)}`;
...
function getFirstList() {
    let memoUrl_first;
    if (memo.APIVersion === 'new') {
        // 适配 Memos 0.30.0：pageSize -> page_size，响应是 {memos:[...], nextPageToken:"..."}
        memoUrl_first = memoUrl + '&page_size=' + limit;
...
function getNextList() {
    if (memo.APIVersion === 'new') {
        // 适配 Memos 0.30.0：pageSize->page_size, pageToken->page_token, 响应包一层 {memos:[...]}
        var memoUrl_next = memoUrl + '&page_size=' + limit + '&page_token=' + encodeURIComponent(nextPageToken);
...
function updateHTMl(data) {
    ...
    for (var i = 0; i < data.length; i++) {
        var uId;
        if (memo.APIVersion === 'new') {
            // 取完整name字段 memos/kmaxqsF8vAgkzJpJvWW3nU
            uId = data[i].name
        } else if (memo.APIVersion === 'legacy') {
            uId = data[i].id
        } else {
        ...
        // 解析内置资源文件
        if (memo.APIVersion === 'new') {
            // 适配 Memos 0.30.0：resources[] -> attachments[]；字段 externalLink -> external_link
            if (data[i].attachments && data[i].attachments.length > 0) {
                ...
                for (var j = 0; j < resourceList.length; j++) {
                    ...
                    var resexlink = resourceList[j].external_link;
                    var resLink = '';
                    var filename = resourceList[j].filename;
                    // 0.30.0 name 形如 "attachments/{uid}"，文件下载路径为 /file/attachments/{uid}/{filename}
...
function getTotal() {
    ...
    let totalUrl;
    if (memo.APIVersion === 'new') {
        // 适配 Memos 0.30.0：name 必须为 users/{user} 资源格式，字段 totalMemoCount -> total_memo_count
        pageUrl = `${memos}/api/v1/users/${memo.creatorId}:getStats`;
        fetch(pageUrl)
            .then(res => res.json())
            .then(resdata => {
                // 直接使用返回数据中的 total_memo_count 字段（0.30.0 snake_case）
                var allnums = (resdata && resdata.total_memo_count !== undefined)
                    ? resdata.total_memo_count
                    : (resdata && resdata.totalMemoCount);
                if (allnums !== undefined && allnums !== null) {
                    ...
                    throw new Error('No valid total_memo_count found');
                }
            })
            .catch(err => {
