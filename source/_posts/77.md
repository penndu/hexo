...
> CGI 是通用网关接口，是用于让交互程序和 Web 服务器通信的协议。FastCGI 是基于 CGI 的增强版本协议，不同于创建新的进程来服务请求，使用持续的进程和创建子进程来处理一连串的进程，这些进程交由 FastCGI 服务器管理，开销更小，效率更高。PHP-FPM 是 PHP 实现的 FastCGI 进程管理器，用于替换 FastCGI 大部分附加功能，适用于高负载网站。
...
这里先说一下涉及到这个的几个参数，它们分别是 pm/pm.max_children/pm.start_servers/pm.min_spare_servers 和 pm.max_spare_servers：
...
实际上和 Apache 一样，我们运行的 PHP 程序在执行完成后，或多或少都会有内存泄露的问题。
...
至于 pm.min_spare_servers，则建议根据服务器的负载情况来设置，比较合适的值在 5 到 10 之间。
