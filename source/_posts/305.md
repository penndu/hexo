...
我们以 /dusays/ 目录为例，使用命令 `/backstores/fileio create dusays /dusays/dusays.img 1G` 创建大小为 1G、名为 dusays 的存储文件，创建后通过 `ls` 查看执行结果：
...
接下来我们创建名为 iqn.2021-01.com.dusays:www 的 iSCSI 目标，一般命名规则为 iqn.年份-月份.网址倒序:存储文件名称，完整命令为 `/iscsi create iqn.2021-01.com.dusays:www`，创建后通过 `ls` 查看执行结果：
