...
我们经常使用如下命令远程控制 Linux 系统：

```
ssh USERNAME@HOSTNAME
```

在此条命令中，系统会自动连接服务器的 22 端口，因为 SSH 的默认端口是 22。如果我们修改默认端口，同样可以增强系统的安全性。首先修改配置文件：
...
每一个 Linux 系统，都会有 root 用户，就像 Windows 系统都有 Administrator 一样。当黑客们尝试破解您服务器的密码时，他们肯定会尝试使用此用户名登录，这样他们只需破解密码即可。那如果我们禁用了 root 用户而改用其它用户，那破解的难度就增加了。首先修改配置文件：
