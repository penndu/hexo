...
> Cloudreve 除了支持 Docker 容器部署，官方还提供单服务部署方式。操作十分简单，仅需要一个文件即可搭建运行 Cloudreve。本文介绍 Cloudreve 的部署过程，以及通过注册服务或守护进程方式实现后台运行。

<!-- more -->

## 部署过程

Linux 下，直接解压并执行主程序即可。这里以 `3.8.3` 版本为例，首先到官网下载压缩包，解压获取到主程序：
...
通过上面的方式运行 Cloudreve，会一直占用着终端。为了不影响终端的使用，我们可将 Cloudreve 注册为服务，这样即使服务器重启后，Cloudreve 也可以自动启动。使用如下命令编辑配置文件：
