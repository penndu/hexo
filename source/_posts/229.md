...
从上面的拓扑中不难看出，我们将几个路由器分隔开，划分成不同区域，为什么要这么做呢？

我们知道，RIP 通过宣告网段，让其它路由器通过学习了解路由走向。而宣告出去的数据，会通过链路传递到网络中的各个设备，网络愈加庞大，要传播的数据也就越多，这样无疑会加大网络的耗损。

这时我们可以通过 OSPF 来划分区域，让路由宣告的数据只在区域内部传递，从而减少网络的耗损。
