...
注意：这个 `"runner": {"name": "activebackup"}` 两边不要有空格，否则会导致配置失败。
...
CentOS 7 中的链路聚合有两种模式，一种是 activebackup 连接冗余，指当一个网卡出现故障时，会自动切换到另外一个网卡。

另一种 roundrobin 负载均衡，会把网络流量分配给两个网卡一并处理，理论上可增加端口带宽，不过需要交换设备支持。