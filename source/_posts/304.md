...
SCSI 是一种 I/O 技术，规范了一种并行的 I/O 总线和相关的协议，SCSI 数据传输是以块的方式进行的。
...
1. iSCSI Initiator 客户端，即 iSCSI 启动器。本质上说，iSCSI 启动器是一个客户端设备，用于将请求连接并启动到服务器。iSCSI 的启动器有三种实现方式，可以完全基于硬件实现，比如 iSCSI HBA 卡，硬件 TOE 卡与软件结合的方式，完全基于软件实现，而且软件 iSCSI 启动器适用于大部分主流操作系统平台；
