...
> 本博客的模板是 Volantis，开发者们非常勤劳，经常更新些新玩意。每次收到更新通知时杜老师都会第一时间同步代码，不过还要登录服务器部署才能生效。本文分享如何配置接收 GitHub Webhook 后自动执行脚本。
...
下载后可先在本地解压再传到服务器的 `/opt` 目录，或在服务器上运行 `unzip linux-webhook.zip`。如提示 `command not found`，请执行下面的命令：
...
{% image https://cdn.dusays.com/2022/05/464-1.jpg  ratio:1918/867 %}

Add webhook:

{% image https://cdn.dusays.com/2022/05/464-2.jpg  ratio:1918/866 %}

Payload URL 填写 `http://IP:PORT/web-hook`，Content type 选择 `application/json`，Secret 填写之前设置的值，最后点击 Add webhook 即可：

{% image https://cdn.dusays.com/2022/05/464-3.jpg  ratio:1918/867 %}

Recent Deliveries 会显示回调状态，绿色对勾即为成功：

{% image https://cdn.dusays.com/2022/05/464-4.jpg  ratio:1918/866 %}
