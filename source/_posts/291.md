...
> 杜老师喜欢使用 Docker 部署轻量级的应用，但后期维护起来还是比较麻烦的，主要是新版本发布后的更新问题——需要停止并删除当前容器，更新镜像后再使用相同参数重新运行。今天为大家介绍一款可以自动更新 Docker 容器的工具！

<!-- more -->

## 工具介绍

Watchtower 是一个应用程序，它会监视正在运行的 Docker 容器，并跟踪这些容器最初启动所用镜像的变更。如果 Watchtower 检测到镜像的版本已更改，它将自动使用新版镜像重新启动容器。

只需要将新镜像推送到 Docker Hub 或自己的镜像注册表，即可更新容器化应用的运行版本。Watchtower 会拉取新版镜像，正常关闭现有容器，并使用与最初部署时相同的选项重新启动它。
