...
若无服务器可以不用搭建 Memos，借助现有平台「如杜老师的 [https://dusays-memos.itisn.cyou/](https://dusays-memos.itisn.cyou/)」注册账户即可。
...
fetch(bbUrl).then(res=>res.json()).then(resdata=>{if(resdata.data){var allnums='<div id="bb-footer"><p class="bb-allpub"><a href="https://dusays-memos.itisn.cyou/" target="_blank">杜老师的说说</a></p></div>'
...
注意：将上面的代码保存为 `dusays.js` 文件，其中的 `dusays` 可自定义，下文内容会使用到。此篇保存目录以`/source/shuoshuo/`为例。
...
<script>
  function getTotal() {
    var totalUrl = "https://dusays-memos.itisn.cyou/api/memo/amount?userId=1";
...
注意：上面代码的 `https://dusays-memos.itisn.cyou/api/memo/amount?userId=1` 一句，`dusays-memos.itisn.cyou` 替换成自己 Memos 的域名，结尾的 `1` 改为用户的 ID。
...
注意：上面代码的 `bber` 可自定义，不要与整站 div 重名；`dusays-memos.itisn.cyou` 替换成自己 Memos 的域名；limit 为展示数量限制，creatorId 对应用户 ID，domId 需与 div id 相同。