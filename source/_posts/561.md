...
注意：上面代码的 `https://dusays-memos.itisn.cyou/api/memo/amount?creatorId=1` 一句，`dusays-memos.itisn.cyou` 替换成自己 Memos 的域名，结尾的 `1` 改为用户的 ID。
...
注意：上面代码的 `dusays` 可自定义，不要与整站 div 重名；`dusays-memos.itisn.cyou` 替换成自己 Memos 的域名；limit 为展示数量限制，creatorId 对应用户 ID，domId 需与 div id 相同。