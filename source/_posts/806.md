...
RSSHub 是一个轻量、易于扩展的 RSS 生成器，它基于 Node.js 开发，支持多种平台和类型的订阅源，包括但不限于微博、知乎、B 站、GitHub 等。
...
如果使用 Follow 调用 RSSHub 实例，需在 `docker-compose.yml` 中添加一些变量。其中 `FOLLOW_OWNER_USER_ID` 是指您的关注账户 ID 或用户名；`FOLLOW_DESCRIPTION` 指实例描述；`FOLLOW_PRICE` 指实例月费，如设为 0 表示免费；`FOLLOW_USER_LIMIT` 指实例用户限制，将其设置为 0 或 1 可将实例设为私有，留空则表示无限制；`ACCESS_KEY` 指访问密钥：
...
{% image https://cdn.dusays.com/2025/03/806-1.jpg  ratio:1918/866 %}