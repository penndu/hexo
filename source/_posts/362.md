...
> 用了一段时间最新版的模板后，发现问题还挺多。虽说人生贵在折腾，但踏踏实实写文章不香吗？所以杜老师将模板从测试版回退到正式版，重新部署时报错，发现是 Node.js 版本导致的。今天就来说一下如何通过 npm 调整 Node.js 版本。
...
注意：在使用 `hexo g` 部署博客页面时，报出上述错误。虽然提示为错误，但实际只是警告信息，如不在意可以忽略。但杜老师有强迫症，一定要解决该问题！
