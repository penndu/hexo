...
> YAML 的语法与其他高级语言类似，并且能够简洁地表达清单、散列表等数据形态。它使用空白符号缩进，并大量依赖外观，特别适合用来表达或编辑数据结构、各种配置文件、转储调试内容以及文件大纲。
...
即 companies 的属性是一个数组，每个数组元素由 id、name、price 三个属性构成。
