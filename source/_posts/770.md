...
OpenResty 通过集成 Nginx 并利用其模块化特性支持 QUIC。需要在 Nginx 配置文件中启用 QUIC 支持，这通常涉及配置监听 UDP 端口，因为 QUIC 运行在 UDP 上。
...
这里的 `443` 和 `444` 是用于 QUIC 的 UDP 端口，`quic` 指定 QUIC 协议，`reuseport` 参数则是为了提高性能：
...
{% image https://cdn.dusays.com/2024/11/770-1.jpg  ratio:1918/866 %}