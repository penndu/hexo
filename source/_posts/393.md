...
这种方式虽然简单粗暴，但也会带来一些弊端，例如：通过 .gitlab-ci.yml 脚本进行文件操作要格外小心。

另外要做好服务端安全设置，确保不被恶意用户获取到操作权限。

## 操作步骤

通过下面命令可查看 gitlab-runner 的工作目录、默认用户等一系列相关信息：

```
ps aux | grep gitlab-runner
```

通过下面命令可卸载 gitlab-runner 默认用户：
