...
> 我们在搭建网站时，为了便于管理，常常选用 PHP+MySQL 这种架构，Web 引擎可选择 Apache 或 Nginx。今天杜老师说一下，选择 Nginx 时，主配置文件和站点配置文件如何改写！
...
注意：以上为 Nginx 主配置文件参考，前提是已安装相关模块。日志目录为 /www/wwwlogs/，Nginx 安装目录为 /www/server/nginx/，站点配置文件目录为 /www/server/panel/vhost/nginx/。
