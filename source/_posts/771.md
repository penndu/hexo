...
> 杜老师不蒜子统计平台运营至今，经过星辰童鞋多轮版本迭代，增加了很多新功能。更为值得称赞的是，版本升级并没有 BREAKING CHANGE，新版本仍兼容旧版本的调用脚本。使用旧版本的小伙伴不升级脚本也可继续使用！
...
############################# Analytics ############################# > start
analytics:
  busuanzi: https://jsd.dusays.com/npm/penndu@17.0.0/bsz.js # 修改这里即可
############################# Analytics ############################# > end
```
...
```
script(async data-pjax src='//jsd.dusays.com/npm/penndu@17.0.0/bsz.js')
```
...
```
<script defer pjax src="https://jsd.dusays.com/npm/penndu@17.0.0/bsz.js"></script>
```

可使用 `data-api` 属性, 指定后端 API 的接口：

```
<script defer data-api="https://bsz.dusays.com:9001/api" src="https://jsd.dusays.com/npm/penndu@17.0.0/bsz.js"></script>
```

可使用 `data-style` 属性, 指定数据的显示样式。默认显示完整数据，`short` 显示短形式, 如 1024 显示为 1k；`comma` 以逗号分隔数据, 如 1024 显示为 1,024：

```
<script defer data-style="short" src="https://jsd.dusays.com/npm/penndu@17.0.0/bsz.js"></script>
```

不同于原版本, 为了更加精简去除了 `value` 字符, 仍然可以通过指定 `data-prefix` 属性来进行兼容：

```
<script defer data-prefix="busuanzi_value" src="https://jsd.dusays.com/npm/penndu@17.0.0/bsz.js"></script>
```