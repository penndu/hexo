...
> 首先完成准备工作，然后安装 DHCP/TFTP 服务端，接着提供 BootLoader 及配置文件，挂载光盘并将内核文件复制到 TFTP 目录，部署 httpd 服务端提供文件服务，最后设置启动菜单并提供系统安装软件等。
...
{% image https://cdn.dusays.com/2020/09/260-1.jpg  ratio:1918/866 %}

流程图示如下：

{% image https://cdn.dusays.com/2020/09/260-2.jpg  ratio:1918/866 %}

## 流程解读

1. 客户端向 PXE 服务器上的 DHCP 发送 IP 地址请求消息，DHCP 检测客户端是否合法，同时将 PXE 环境下 BootLoader 文件 pxelinux.0 的位置信息发送给客户端；

2. 客户端向 PXE 服务器上的 TFTP 请求 pxelinux.0，TFTP 收到消息后向客户端发送 pxelinux.0 的大小信息，同时等待确认；当 TFTP 收到客户端发回的确认大小信息后，正式发送 pxelinux.0；

3. 客户端执行接收到的 pxelinux.0；

4. 客户端向 TFTP 请求 pxelinux.cfg 文件（实际上它是一个目录，里面存放启动菜单，即 GRUB 配置文件），TFTP 会将配置文件发回客户端，继而由客户端根据配置文件执行后续操作；

5. 客户端向 TFTP 发送 Linux 内核请求信息，TFTP 发送 Kernel；

6. 客户端向 TFTP 发送根文件系统请求信息，TFTP 收到消息后返回 Linux 根文件系统；

7. 客户端加载 Linux 内核；

8. 客户端通过 NFS/FTP/HTTP 下载系统安装文件进行安装。如果配置文件指定了 Kickstart 路径，则根据该文件自动应答并完成系统安装。
...
从流程可知，BootLoader 文件 pxelinux.0 以及内核相关的配置文件（即 pxelinux.cfg 目录下的内容）主要由 TFTP 提供：
...
注意：menu.c32 是图形化启动菜单，pxelinux.0 是 BootLoader 文件，pxelinux.cfg 是开机菜单配置目录。
