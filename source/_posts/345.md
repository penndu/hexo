...
前提是环境已安装了 Node.js 和 npm 依赖，如未安装可以参考如下命令，以 Ubuntu 为例：
...
通过终端切换到需要执行格式化的目录下，textlint 支持多层级目录和多文件批量处理，所以一般切换至项目根目录下即可。然后执行下面命令初始化 textlint 针对该目录的配置文件，执行后会自动生成一个名为.textlintrc 的配置文件：
