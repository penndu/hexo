...
在开始分析前，我们需要了解 access_log 的默认日志格式。通常情况下 access_log 的日志格式如下：
...
分析页面的访问时间可以发现哪些页面的加载时间较长，需要优化。在 Nginx 的 `log_format` 中加入 `$request_time` 字段，然后使用以下命令列出传输时间超过 `3` 秒的页面，该命令会输出传输时间超过 `3` 秒的页面及其出现的次数，仅显示前 `20` 条记录：
