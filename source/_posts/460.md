...
> Samba 是在 Linux 和 Unix 系统上实现 SMB 协议的一款免费软件，由服务器及客户端程序构成。Samba 能够在任何支持 SMB 协议的主机之间共享文件，当然也包括 Windows。本文讲解如何在麒麟服务器操作系统上安装 Samba 文件共享服务。

<!-- more -->

## 什么是 SMB

SMB 是一种局域网上的文件和打印机共享通信协议，它为局域网内不同计算机之间提供文件及打印机等资源的共享服务。

SMB 协议是 C/S 型协议，客户机通过该协议可访问服务器上的共享文件系统、打印机及其他资源。

## 监听端口

TCP 端口对应的服务是 smbd 服务，其作用是提供对服务器中文件、打印资源的共享访问；UDP 端口对应的服务是 nmbd 服务，其作用是提供基于 NetBIOS 主机名称的解析：
...
注意：麒麟操作系统官方源中已包含 Samba 安装文件，直接使用上面命令安装即可。
...
注意：`/etc/samba/smb.conf` 是服务端配置文件，刚安装后就自带该文件，直接启动即可使用。
...
注意：可直接使用 Linux 用户，但密码仍需单独设置，使用上述命令可为指定用户设置 Samba 密码。
...
以前的 Samba 版本支持的安全级别有四个，分别是 share/user/server/domain。share 用来设置匿名访问，但现在的版本已不再支持 share，不过仍可通过修改配置实现匿名访问，配置方式发生了变化。首先我们创建共享目录：
...
在 Windows 资源管理器地址栏输入 `\\IP` 即可登录 Samba 服务，如 Samba 服务器 IP 地址是 192.168.1.1，则输入 `\\192.168.1.1`。

输入刚才新增的 Samba 用户名和密码即可。如不想每次都输入，可勾选记住密码。
