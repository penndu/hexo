...
> 有小伙伴留言说想在 Linux 系统上搭建 Minecraft 服务。其实搭建 Minecraft 服务很简单，安装对应版本的 Java，使用命令启动核心文件即可。下面简单整理一下搭建步骤，有需要的小伙伴可参考一下~

<!-- more -->

## 版本对应

本教程以最常用的 Paper 服务端核心为例，下面是版本对应列表：
...
1. 相比官方原版服务端，Paper 的性能优化更好，而且支持插件；

2. 启动后服务端会生成一大堆文件，插件请上传至 `plugins` 目录中；

3. 服务运行期间不可关闭终端，建议通过 `screen` 命令运行；

4. 如使用云主机搭建 MC 服务，记得开启对应端口，端口设置可在 `server.properties` 文件中修改，默认为 25565。
