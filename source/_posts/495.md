...
如 Docker Hub 下载较慢，也可使用下面的阿里云镜像源：
...
管理员进入后台角色组设置，选择指定的角色组点击编辑，进入「图片审核」标签，开启图片审核，设置审核动作，驱动调整为 NsfwJs，接口地址设置为 `http://127.0.0.1:3000/classify`，属性为 `image`，阈值为 `60`「以上参数根据实际情况调整」

{% image https://cdn.dusays.com/2022/08/495-1.jpg  ratio:1920/867 %}