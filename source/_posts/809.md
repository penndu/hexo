...
> 当我们在使用 Linux 系统进行开发或者运维工作时，经常会遇到一些错误提示。其中一个常见错误是解释器权限不够。这个错误提示表明在运行 `targetcli` 命令时，系统没有足够权限来执行所需的 Python 解释器。在本文中，我们将探讨这个错误的原因和解决方法。
...
Linux 系统将用户和组织分为不同的分类，并为每个用户和组织分配不同的权限。这些权限决定了用户可以访问和执行哪些文件和目录。
...
一旦我们找到了 `python3` 解释器的位置，我们就可以通过修改 `targetcli` 脚本来更新解释器的路径。

打开终端并输入命令 `sudo vi /usr/bin/targetcli`。这将使用编辑器打开 `targetcli` 脚本。

在脚本中，我们需要查找`#!/usr/bin/python3` 这一行，并将其替换为 `python3` 解释器的实际路径。

在这个例子中，我们可以将其替换为`#!/usr/local/bin/python3`。
