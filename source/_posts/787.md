...
还需指定日期和时间的格式，以确保 GoAccess 能够正确解析日志文件中的日期和时间信息。例如：
...
在终端直接运行 GoAccess，它会进入一个交互式界面，显示日志分析结果。在交互式界面中可使用键盘方向键和数字键来浏览和查看不同的统计信息。例如：
...
GoAccess 提供了丰富的统计信息和图表，帮助我们全面了解网站访问情况，包括访问概览等：
