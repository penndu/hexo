...
在 CentOS 操作系统上安装 ClamAV，请依次执行以下命令「其中 `clamav` 是扫描工具，`clamavd` 是实时保护服务，`clamav-update` 是毒库升级工具」。
...
在 Ubuntu 操作系统上安装 ClamAV，请依次执行以下命令「其中 `clamav` 是扫描工具，`clamav-daemon` 是实时保护服务」。
