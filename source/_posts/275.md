...
> 该脚本来源于互联网，在此基础上做了一些简单的修改，主要删除了一些无用的代码，但内部测试规则未做任何改动。脚本中使用国外源进行下载速度测试，针对国内主机可能不够准确，结果仅供参考！

<!-- more -->

## 脚本介绍

本脚本可以显示云主机使用的 CPU 型号、主频、缓存大小、硬盘容量和内存等信息，并测试硬盘的读写速度以及下载速度（一般云主机服务商的下行速度不做限制）。

使用方法非常简单：在云主机上使用 vi 命令新建一个文件，将上述脚本复制到文件中并赋予执行权限，直接运行即可。
