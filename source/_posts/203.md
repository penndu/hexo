...
Apache 安装后，默认基于 IP，而且是单 IP 的形式，可直接通过本机的 IP 访问，默认的 `DocumentRoot` 为 /var/www/html，可通过配置文件 /etc/httpd/conf/httpd.conf 修改。
...
注意：如果配置多 IP 的虚拟主机，请为主机增加 IP 后按照上面的流程配置。
