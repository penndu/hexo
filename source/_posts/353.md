...
注意：首先输入 GitLab 的地址，然后复制项目「设置——CI/CD」中获取的 token 值，再输入 runner 的描述以便于区分，接着输入 runner 的 tags（可留空），最后设置执行方式即可。

## 设置

进入 GitLab 项目中，按照下图所示找到 runner 配置文件添加处，参考下面代码进行配置：

{% image https://cdn.dusays.com/2021/06/353-1.jpg  ratio:1918/866 %}
