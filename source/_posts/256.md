...
> Playbook 是一种不同于 Ansible 命令行执行方式的模式，其功能更强大、更灵活。Playbook 支持定制配置，能够按照指定的操作步骤有序执行，同时支持同步与异步方式。需要注意的是，Playbook 通过 YAML 格式进行描述和定义。
...
{% image https://cdn.dusays.com/2020/08/256-1.jpg  ratio:1918/866 %}
...
将所有编写好的 YAML 整合到同一个文件中，便于一次性执行：
