...
> 我们在搭建网站时，为了便于管理，常常选用 PHP+MySQL 这种架构，Web 引擎可选择 Apache 或 Nginx。今天杜老师说一下，选择 Apache 时，主配置文件和站点配置文件如何改写！
...
注意：以上为 Apache 主配置文件参考，前提是已安装相关模块。日志目录为 /www/wwwlogs/，Apache 安装目录为 /www/server/apache/，站点配置文件目录为 /www/server/panel/vhost/apache/。
...
注意：以去不图床的配置文件为例，可自定义项包括：监听端口、站点域名、网站目录、端口转发、证书信息、强制跳转、地址重写、文件缓存、文件屏蔽等。
