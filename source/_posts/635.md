...
> 杜老师今天出差归来，在返京高铁上因为无聊，就看了下近期比较热门的 GitHub 开源项目，正好发现这款工具。可以自动将图片转换为 WebP 格式，同时保持原有的 URL。快点进来了解一下吧！
...
将上方代码中 `./path/to/pics` 改为图片所在的路径，如 `/www/wwwroot/bu.dusays.com`。`./exhaust` 为转换后 WebP 图片保存路径，`./metadata` 为元数据保存路径。配置好后执行如下命令即可：
...
{% image https://cdn.dusays.com/2023/10/635-1.jpg  ratio:1918/866 %}

如果像杜老师一样仅处理 JPEG 格式图像，则可以在站点配置文件中添加如下配置：
