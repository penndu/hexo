...
> Docker 重度使用者们，您了解过 Docker 日志吗？如何查找、减少以及管理日志，从而有效节省磁盘空间！今天分享一下如何处理 Docker 日志问题。
...
主要通过日志轮转方式处理，下面介绍两种方式设置日志轮转。在运行容器时设置轮转策略：
...
禁用日志输出，这个操作慎用，有可能影响应用运行，而且不方便后续的排查：
