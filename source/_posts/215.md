...
> 在部署网络时，我们要考虑到负载均衡和高可用：如何将流量分配到两台设备上；如何在一台设备出现故障时，由另一台设备承担所有流量。这里我们可以通过思科的 VRRP 来实现。
...
终端在发送数据时，会先去找网关，而网关地址是通过 R1/R2 的优先级决定的：优先级高的会抢占虚拟网关 IP，而优先级低的只能等高优先级设备宕机后再抢过来，以此实现高可用性。

而将两台设备的优先级平均分配，就实现了负载均衡。