...
test('doc_tree 按语言过滤页面并回退项目默认配置', () => {
  const fixture = {
    data: {
      'wiki/alpha': {
        name: '默认项目',
        base_dir: '/wiki/alpha/',
        tree: { '默认章节': ['index', 'setup'] },
        locales: {
          en: {
            name: 'English project',
            tree: { 'Getting Started': ['index'] }
          }
        }
      },
      'wiki/beta': { name: '仅中文项目' }
    },
    pages: [
      { _id: 'zh-alpha', wiki: 'alpha', title: '中文首页', path: 'wiki/alpha/index.html' },
      { _id: 'en-alpha', wiki: 'alpha', title: 'English home', path: 'en/wiki/alpha/index.html' },
      { _id: 'zh-beta', wiki: 'beta', title: '中文项目', path: 'wiki/beta/index.html' }
    ],
    shelf: ['alpha', 'beta'],
    siteTree: { index_wiki: { base_dir: '/wiki' } },
    language: 'en',
    context: {
      ...
      theme: { config: { language_switcher: { items: [{ code: 'zh-CN' }, { code: 'en' }] } } }
    }
  };
  const wiki = buildWikiTree(fixture);

  assert.deepEqual(wiki.shelf, ['alpha']);
  assert.deepEqual(wiki.all_pages.map(page => page.title), ['English home']);
  assert.equal(wiki.tree.alpha.name, 'English project');
  assert.deepEqual(wiki.tree.alpha.sections.map(section => section.title), ['Getting Started']);
  assert.equal(wiki.tree.beta.pages, undefined);
});
